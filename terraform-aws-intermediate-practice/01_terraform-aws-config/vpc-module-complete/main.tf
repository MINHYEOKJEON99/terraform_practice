data "aws_availability_zones" "available" {} # 사용가능한 가용 영역 목록을 가져옴
...
locals {
  ...
  tags = {
    Example    = local.name              # 태그의 프로젝트 이름 포함
...
module "vpc" {
  ...
  cidr = local.vpc_cidr # VPC CIDR 블록 설정
  ...
  single_nat_gateway = true # 단일 NAT 게이트웨이 활성화
  ...
  customer_gateways = {
    ...
    IP2 = {
      ...
      ip_address = "5.6.7.8" # 두번째 고객 게이트웨이의 IP 주소 설정
    }
  }
  ...
  vpc_flow_log_iam_policy_use_name_prefix = false                       # IAM 역할 이름 접두사 사용하지 않음
